        ...
        setSelectEvent: function(element){
            ...
            if(that.settings.enable){
                ...
                selectItem.on('click',function(event){
                    // var vacl = $(this).index()
                    // var sign = $(this).parents(".p-content-distribution-time").attr('id')
                    // if(sign == undefined){
                    //     $(this).parents(".p-content-distribution-time").siblings(".zcdata").hide().eq(vacl).show()
                    // }else{
                    //     $(this).parents(".p-content-distribution-time").siblings(".clearfix").children('.zcdata').hide().eq(vacl).show()
                    //     var funcName = 'eduss' + sign + vacl
                    ...
                    $(this)
                        ...
                        var key =  $(this).siblings().parent().parent().siblings('input[type="hidden"]').val()
                        var vacl = $(this).index()
                        var sign = $(this).siblings().parent().parent().siblings('input[type="hidden"]').attr("name");
                        if(sign == 'edu' || sign == 'edus'){
                            $(this).parents(".p-content-distribution-time").siblings(".zcdata").hide().eq(vacl).show()
                        }else if(sign == 'eduss' || sign == 'edusss'){
                            $(this).parents(".p-content-distribution-time").siblings(".clearfix").children('.zcdata').hide().eq(vacl).show()
                            var funcName = sign + vacl
                            if (typeof (eval(funcName)) == "function") {
                                eval(funcName + "();");
                            }
                        }else if(sign == 'edux'){
                            if(key != '0'){
                                var urka = window.location.origin +'/products/'+ key + '/index.html'
                                console.log(urka)
                                window.location.href = urka
                            }
                            console.log(key)
                        }
